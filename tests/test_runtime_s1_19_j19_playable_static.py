...
class T(unittest.TestCase):
 def test_contract(self):
  ...
  self.assertIn("func select_j19_pivot",state);self.assertIn("j19_raphaelle_creative_access_01",state);self.assertIn("fact_pauline_private_state_defined",state);self.assertIn("func restore_snapshot",provider);self.assertIn("const SNAPSHOT_VERSION := 21",season);self.assertIn('"J19": j19_provider.snapshot()',season)
