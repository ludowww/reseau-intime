# R8A — Blueprint de correction de `RouteState`
...
Le verdict est **ARCHITECTURE_APPLICABLE_DANS_SON_PRINCIPE, NON_AUTORISEE_POUR_IMPLEMENTATION, SCHEMA_FINAL_CONDITIONNE_PAR_LE_CONTRAT_NARRATIF_DE_SAISON**. Le moteur reste une proposition de conception : il ne peut devenir implémentable qu’après signature du contrat narratif de saison, notamment de la finale Marie/Player.
...
|---|---|
| Baseline R8A-C1 | `0d7247e7f671e7dfea212f4db23730aad365b87c` |
| `main` et `origin/main` | même commit |
| Tag de baseline | `runtime-r7a-c2-j16-j21-provider-snapshot-contracts` |
| État initial du worktree | propre, `HEAD` détachée avant création de la branche de documentation |
| Branche de travail | `work/r8a-c1-route-state-blueprint-corrections` |
...
Ordre d’autorité obligatoire en cas d’écart : **décisions produit et scripts canoniques signés récents** > **registres et contrats canoniques** > **amendements applicables** > **runtime verrouillé**, qui n’est qu’un constat d’implémentation > **tests**, qui prouvent seulement le comportement implémenté > documentation historique. Un comportement testé peut être un bug ; le runtime ne devient jamais canon automatiquement.
...
| F | historique ou conclusion dérivée | reconstituer proprement depuis événement, fait ou registre après décision produit |
| G | legacy probablement inutilisé | conserver comme constat d’audit ; supprimer uniquement si la reconstruction le rend inutile |
| H | sens ou ownership à tracer | classifier explicitement avant reconstruction |
...
- `household_rhythm_confirmed` : lecteurs tracés dans J04 (écriture), J04/J14 (gates et smokes) et les contrats de scènes. Classification retenue : **fait de monde/foyer persistant**, sourcé par l’événement de clôture du foyer J04. Il signifie que le rythme domestique d’ouverture est établi ; il ne prouve jamais la présence de Marie près d’un écran en J14.
- `j06_external_continuity_resolution` : écrivains tracés dans les résolutions J06 (`NO_PROMISE`, `UNAVAILABLE`, `EXPIRED`) ; lecteurs dans les gates J06, le sélecteur J10 et les fixtures J07/J08/J10/J12. Classification retenue : **événement historique de résolution externe**, avec conclusion dérivée par les règles. Il ne doit plus être un scalaire courant ni rester indéterminé : le futur modèle conserve l’événement source, et calcule la conclusion requise.

### Trace narrative, événement et connaissance

Une `trace_narrative` est un contenu, objet ou enregistrement persistant dont l’existence, le contrôle, l’audience, la conservation, la circulation, l’accessibilité ou la suppression peuvent produire des conséquences. Toute action ne crée pas une trace.

Exemple : Pauline envoie une photo privée. L’**événement** est `photo_privee_envoyee` à un instant donné ; la **trace narrative** est la photo, avec son contrôleur, son audience et son état de conservation ; la **connaissance** est « Marie sait que cette photo existe », éventuellement sans y avoir accès. Supprimer la photo modifie la trace ; cela n’efface ni l’événement ni la connaissance déjà acquise.
...
10. **Reconstruction, pas migration naïve.** Le jeu étant en développement, la rupture des sauvegardes v1–v25 est autorisée. Il n’y a ni double architecture permanente, ni double écriture longue ; le runtime legacy sert temporairement d’oracle fonctionnel pour comparer les comportements, jamais de schéma à pérenniser.

Le schéma exclut explicitement `affection_score`, `lust_score`, `corruption_score`, `route_points` et `consent_score`.

## 10. Modèle cible en vocabulaire français
...
EtatNarratif
  progression_saison
  relation_centrale: EtatRelationCentrale
  relations[personnage_id]: EtatRelation
  evenements
  promesses
  obligations
  traces_narratives
...
Les identifiants conceptuels et techniques sont en français sans accents, avec `snake_case` recommandé. `etat_arc` reste spécifique au personnage ; les axes qualitatifs communs ne sont ni des scores ni des permissions. Pour Nico/Player, `desir = NONE`. Les préférences et désirs du joueur sont des vues calculées depuis événements et choix, jamais un profil figé ou un score persistant. `mensonge` n’est jamais un `contrat_couple` : c’est un `etat_divulgation`.

La relation centrale Marie/Player est distincte de chaque `EtatRelation`. Elle doit pouvoir conclure la saison par une conversation obligatoire : rester ensemble (contrat `exclusif`, `ouvert` ou `libertin`, avec `etat_divulgation` séparé) ou se séparer (relation résiduelle `bons_termes`, `blessee`, `hostile` ou `sans_contact`). C’est une capacité obligatoire du futur moteur, pas une réécriture immédiate de la Bible narrative.
...
ContexteScene
  scene_id
  personnage_ids
  conditions_temporaires
  consentement_actuel: NON_DEMANDE | INCERTAIN | CONFIRME | REFUSE | RETIRE
  preuves_presentees
  effets_en_attente
```

`consentement_actuel` est exclusivement local à `ContexteScene`. Il ne survit jamais comme permission sexuelle persistante, ne se déduit ni du désir ni de l’intimité, et est oublié à la clôture de la scène.
...
`NON_DEMANDE`, `INCERTAIN`, `CONFIRME`, `REFUSE`, `RETIRE`.

Le consentement est évalué pour une action et un contexte précis. Il n’est ni copié dans `EtatRelation`, ni dérivé de `desir`, ni reconstruit depuis une intimité passée. Après restauration au milieu d’une scène, le provider doit prouver que l’énoncé ou le geste a réellement été présenté ; sinon `consentement_actuel` revient à `NON_DEMANDE` ou la branche ferme.
...
La relation centrale Marie/Player ne doit pas absorber toute la vérité du foyer. Les faits de logement, présence et calendrier restent des événements ou faits de monde référencés par les règles. Les liens entre personnages secondaires ne constituent pas une matrice universelle : un événement, fait ou état relationnel interpersonnel n’est créé que lorsqu’une continuité durable le rend narrativement utile.

## 13. Règles de scènes déclaratives

Une scène modulaire est une unité réutilisable de présentation, de choix et de production d’événements. Elle peut être légère, relationnelle ou structurante, et peut incarner un événement du tronc commun. Elle ne possède aucune vérité relationnelle persistante et ne crée aucun champ `jNN_*`. Les providers et scènes ne modifient jamais directement `EtatRelation` ni `EtatRelationCentrale`.

Seuls `ReducerRelation` et `RouteReducer` peuvent modifier ces deux états. Toute transition est déclenchée par événement, atomique, validée contre les combinaisons autorisées et rejetée si incohérente. Les conséquences spécialisées appartiennent aux registres et reducers correspondants.

`MoteurReglesScene` / `SceneRuleEngine` calcule l’éligibilité depuis les états, événements, registres et faits ; il ne persiste rien. Une règle de scène décrit donc l’éligibilité et les effets à remettre au reducer ; son évaluation ne mute aucun état. La façade d’auteur contient au minimum :
...
`scene.consent_is` n’est évalué qu’après la demande actuelle dans `ContexteScene.consentement_actuel`. Les trois preuves matérielles ne remplacent pas ce consentement.
...
| Consentement actuel | paramètre `consent_current` de la fonction J11 | `ContexteScene.consentement_actuel=CONFIRME` pour l’action précise ; jamais dans l’état relationnel |
...
## 16. Reconstruction par tranches de dépendances

Ce plan est futur ; aucune phase n’est implémentée par ce blueprint.

| Tranche | Objet | Précondition |
|---|---|---|
| Fondation commune | `EtatNarratif`, registres, reducers, moteur de règles et contrats d’événements | contrat de saison signé |
| Actes I–II J01–J09 | progression, foyer, premières relations et événements historiques | fondation validée |
| Pivot J10–J13 | décisions de pivot, règles et traces | actes I–II stables |
| Découverte et conséquences J14–J16 | connaissances, divulgation, obligations et preuves | pivot validé |
| Résolutions J17–J21 | relation centrale, conclusions et conversation finale obligatoire | contrat de finale signé |

Pauline est le pilote conceptuel et Mathilde la validation critique ; ce ne sont pas des migrations techniques personnage par personnage. Chaque tranche reconstruit un modèle cohérent ; le legacy n’est consulté que comme oracle fonctionnel temporaire. Les sauvegardes v1–v25 peuvent rompre au cutover : aucune compatibilité, double écriture longue ou seconde architecture durable n’est requise.

### Règles de reconstruction

- La reconstruction est déterministe et explicitement bornée par le contrat narratif signé ; elle ne déduit aucune valeur favorable d’une archive ambiguë.
...
- Aucun champ `jNN_*` n’est créé dans le nouveau modèle ; les faits, événements et registres remplacent les scalaires historiques lorsque leur rôle est nécessaire.
...
| Reconstruction divergeant de l’oracle legacy | Moyenne | Élevé | comparaison temporaire, puis remplacement net | corpus de comportements validés |
...
| Régression de reprise provider | Moyenne | Élevé | reconstruction indépendante de la compaction provider | snapshots de toutes les phases J14/J15 |
...
1. Adopter `EtatNarratif`, `EtatRelation`, `EtatRelationCentrale` et le registre d’événements, sans score relationnel.
2. Garder `promises`, `obligations`, `traces` et `knowledge` comme registres spécialisés autoritatifs.
3. Considérer les listes d’identifiants de l’état relationnel comme des index reconstructibles.
4. Valider les enums communs des sections 10–11, notamment la distinction M-B1/M-B2/M-B3 par `intimacy` **et** événement.
5. Valider Pauline comme pilote conceptuel et Mathilde comme validation critique obligatoire, sans imposer une migration technique personnage par personnage.
...
7. Retenir les classifications explicites de `household_rhythm_confirmed` et `j06_external_continuity_resolution` de la section 8.
8. Ne pas coupler la reconstruction de l’état narratif à une éventuelle compaction des snapshots de providers.
...
| R8F | reconstruction finale par tranches | contrat narratif de saison signé |
...
- les archives legacy ne servent qu’à la comparaison fonctionnelle, sans valeur favorable inventée ;
...
- le nouveau modèle ne conserve aucune double architecture ni double écriture durable ;
...
Verdict final : **ARCHITECTURE_APPLICABLE_DANS_SON_PRINCIPE, NON_AUTORISEE_POUR_IMPLEMENTATION, SCHEMA_FINAL_CONDITIONNE_PAR_LE_CONTRAT_NARRATIF_DE_SAISON**.
