...
func _run() -> void:
	...
	var private_image = messages.conversation_screen.timeline.image_messages[0]
	_expect(not private_image.has_loaded_texture(), "PROTOTYPE ImageMessage must not load a texture")
	_expect(private_image.displayed_media_status() == "DEVELOPMENT_PLACEHOLDER", "PROTOTYPE ImageMessage status mismatch")
	_expect(private_image.image_button.text == "Visuel non livré", "PROTOTYPE ImageMessage fallback mismatch")
	var empty_image = preload("res://scripts/ui/messages/ImageMessage.gd").new()
	empty_image.configure("empty_image", "", "", Color.WHITE, shell.PORTRAIT_THEME, "Ancien placeholder")
	...
	_expect(forwarded_message_id == "demo_image_private_marie_01" and forwarded_media_ref == "marie_tuesday_black_dress_mirror_01", "private signal must preserve internal identifiers")
	...
	_expect(not shell.photo_viewer.has_loaded_texture(), "private PROTOTYPE viewer must not load a texture")
	_expect(shell.photo_viewer.displayed_media_status() == "DEVELOPMENT_PLACEHOLDER", "private PROTOTYPE viewer status mismatch")
	_expect(shell.photo_viewer.visual_label.text == "Visuel non livré" and shell.photo_viewer.visual_label.visible, "private PROTOTYPE viewer fallback mismatch")
	...
	var group_image = messages.conversation_screen.timeline.image_messages[0]
	_expect(not group_image.has_loaded_texture() and group_image.displayed_media_status() == "INVALID_REFERENCE", "invalid ImageMessage reference must not crash or load")
	_expect(group_image.image_button.text == "Visuel non livré", "invalid ImageMessage fallback mismatch")
	_validate_image_geometry(messages)
	...
	_expect(not shell.photo_viewer.has_loaded_texture() and shell.photo_viewer.visual_label.text == "Visuel non livré", "invalid viewer reference must use explicit fallback")
	shell._close_photo_viewer()
	await _settle(false)
