...
func _exercise_current_snapshot_policy() -> void:
	var state = _completed_network_state(true)
	var provider = _new_provider(state)
	_expect_state_round_trip(state, "current J13 handoff")
	_expect_round_trip(provider, "current J13 before start")
	_expect(bool(provider.start_day().get("accepted", false)), "current J13 fixture starts")
	_expect_round_trip(provider, "current J13 before delivery")
	_confirm(provider)
	_present(provider, "thread_pauline_private")
	_expect(bool(provider.apply_choice("thread_pauline_private", "choice_j13_pauline_rule").get("accepted", false)), "current J13 fixture resolves Pauline")
	_expect_round_trip(provider, "current J13 after choice")
	_expect_state_round_trip(state, "current J13 state after choice")
	for obsolete_version in [1]:
		var obsolete_provider: Dictionary = provider.snapshot()
		obsolete_provider["version"] = obsolete_version
		_expect_v2_snapshot_rejected(state, obsolete_provider, "obsolete J13 provider v%s is rejected" % obsolete_version)
	for obsolete_version in [22, 24]:
...
func _exercise_v2_visual_snapshot_fail_closed() -> void:
	...
	var wrong_asset := canonical.duplicate(true); _message_in_snapshot_by_id(wrong_asset, "thread_pauline_private", PAULINE_PHOTO_MESSAGE)["asset_id"] = "WRONG"
	_expect_v2_snapshot_rejected(state, wrong_asset, "v2 wrong visual asset fails closed")
	var missing_asset := canonical.duplicate(true); _message_in_snapshot_by_id(missing_asset, "thread_pauline_private", PAULINE_PHOTO_MESSAGE).erase("asset_id")
	_expect_v2_snapshot_rejected(state, missing_asset, "v2 missing visual asset fails closed")
	var wrong_message := canonical.duplicate(true); _message_in_snapshot_by_id(wrong_message, "thread_pauline_private", PAULINE_PHOTO_MESSAGE)["message_id"] = "msg_j13_pauline_photo_wrong"; wrong_message["produced_message_ids"].erase(PAULINE_PHOTO_MESSAGE); wrong_message["produced_message_ids"]["msg_j13_pauline_photo_wrong"] = true
	_expect_v2_snapshot_rejected(state, wrong_message, "v2 wrong visual message id fails closed")
	var wrong_label := canonical.duplicate(true); _message_in_snapshot_by_id(wrong_label, "thread_pauline_private", PAULINE_PHOTO_MESSAGE)["placeholder_label"] = "WRONG"
	...
	var invalid_photo := canonical.duplicate(true); _message_in_snapshot_by_id(invalid_photo, "thread_pauline_private", PAULINE_PHOTO_MESSAGE)["content_type"] = "PHOTO"
	_expect_v2_snapshot_rejected(state, invalid_photo, "v2 injected PHOTO presentation fails closed")
	...
	var duplicate := canonical.duplicate(true); duplicate["transcripts_by_thread"]["thread_pauline_private"].append(_message_in_snapshot_by_id(duplicate, "thread_pauline_private", PAULINE_PHOTO_MESSAGE).duplicate(true))
...
func _message_in_snapshot_by_id(snapshot: Dictionary, thread_id: String, message_id: String) -> Dictionary:
