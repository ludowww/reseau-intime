...
func _exercise_real_portrait_path() -> void:
	...
	await _wait_until(func(): return provider.active_day == "J06" and provider.j06_provider.phase == "mathilde_incoming" and not messages.transition_flow_active, 480, "J05 to J06 handoff timed out")
	_expect(provider.state.sandra_j05_outcome == "THREAD_MAINTAINED", "safe Sandra choice records THREAD_MAINTAINED")
	_expect(provider.content_end().is_empty(), "J05 no longer exposes CONTENT_END")
	_expect(messages.screen_mode == "list" and messages.thread_has_unread_content("thread_mathilde_private"), "J06 starts on the list with Mathilde unread")
