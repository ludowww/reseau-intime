...
func _run() -> void:
	...
	_expect(not shell.photo_viewer.has_loaded_texture() and shell.photo_viewer.displayed_media_status() == "DEVELOPMENT_PLACEHOLDER", "private PROTOTYPE viewer must use the development placeholder status")
	_expect(shell.photo_viewer.visual_texture.stretch_mode == TextureRect.STRETCH_KEEP_ASPECT_CENTERED, "viewer texture must preserve its aspect ratio")
	_expect(shell.photo_viewer.displayed_texture_ratio() == 0.0, "PROTOTYPE viewer must not expose a texture ratio")
	_expect(shell.photo_viewer.visual_label.text == "Visuel non livré" and shell.photo_viewer.visual_label.visible, "PROTOTYPE viewer must use the delivery fallback")
	...
	_expect(not shell.photo_viewer.has_loaded_texture() and shell.photo_viewer.visual_label.text == "Visuel non livré", "missing gallery full_ref must remain identifiable")
	shell._close_photo_viewer()
	await _frames(5)
	...
	_expect(not shell.photo_viewer.has_loaded_texture() and shell.photo_viewer.displayed_media_status() == "DEVELOPMENT_PLACEHOLDER", "gallery PROTOTYPE full_ref must remain non-delivered")
	_expect(shell.photo_viewer.visual_label.text == "Visuel non livré", "gallery PROTOTYPE full_ref fallback mismatch")
