...
func _exercise_real_portrait_path(size: Vector2i) -> void:
	...
	await _wait_until(func(): return provider.j07_provider.phase == "complete" and messages.is_day_transition_active(), 600, "J07 CONTENT_END timed out")
	_expect(str(provider.content_end().get("transition_mode", "")) == "CONTENT_END", "CONTENT_END moved to J07")
	_expect(str(messages.day_transition.display_title()) == "J07 terminé", "J07 final card visible")
