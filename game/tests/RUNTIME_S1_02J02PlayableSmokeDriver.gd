...
func _run() -> void:
	...
	_expect(provider.active_day == "J02", "handoff must select and start J02 automatically")
	...
	_expect(not messages.day_transition.visible, "J02 end and J03 start cards must be removed")
	...
	_expect(provider.active_day == "J03", "J02 must hand off automatically to J03")
	_expect(not messages._thread_for("thread_raphaelle_private").is_empty(), "J03 thread missing after automatic handoff")
	_expect(_gallery_asset_count(provider.gallery_source(), "S1_A1_J02_SCN_FIRST_SHARED_EVENING_01") == 2, "shared J02 gallery asset changed during handoff")
	_expect(provider.state.current_day == "J03" and provider.state.day_status == "ACTIVE", "J03 must start automatically")
...
func _run_ui_outcome(marie_choice: String, mathilde_choice: String, expected_status: String) -> void:
	...
	await _play_j01(messages)
	_expect(provider.active_day == "J02", "alternate UI did not automatically complete J01")
	await _open(messages, "thread_marie_private")
	await _choose(messages, "choice_wed_marie_emergency_guided")
	...
	_expect(_message_text(messages, "thread_mathilde_private", offline_id) == offline_text, "exact branch offline beat text mismatch: %s" % mathilde_choice)
