...
func _run() -> void:
	...
	overlay.set_speed_multiplier(1.0)
	overlay.set_speed_multiplier(3.0)
	overlay.set_speed_multiplier(8.0)
	_expect(overlay.speed_multiplier == 8.0, "×1 / ×3 / ×8 share one speed state")
	await overlay.flow_finished
	overlay.dismiss()
...
func _exercise_overlay_robustness(overlay) -> void:
	...
	var before_speed_change := _time_minutes(overlay.clock_label.text)
	overlay.set_speed_multiplier(1.0)
	overlay._process(0.05)
	_expect(_time_minutes(overlay.clock_label.text) >= before_speed_change, "speed change remains monotone")
	_send_key(KEY_ENTER)
	await _frames(2)
	...
	_expect(overlay.active, "reduced motion remains perceptible at ×8")
	...
	overlay.set_reduced_motion(false)
	overlay.set_speed_multiplier(8.0)
	_expect(overlay.play_flow([{"phase": "OFF_PHONE", "text": "Lecture accélérable.", "duration_seconds": 5.0}], 93), "off-phone keyboard flow starts")
	overlay.set_process(false)
	_send_key(KEY_SPACE)
	await _frames(2)
