...
func _exercise_mathilde_physical_aftercare() -> void:
	...
	_expect(provider.state.j11_pivot_outcome == "MATHILDE_M_B3", "Mathilde M-B3 records an exact semantic outcome")
...
func _exercise_mathilde_lower_ceilings() -> void:
	...
	_expect(b2.state.j11_pivot_outcome == "MATHILDE_M_B2", "M-B2 semantic outcome remains distinct from M-B3")
	...
	_expect(stopped.state.j11_pivot_outcome == "MATHILDE_CLEAN_STOP", "clean stop has its own semantic outcome")
...
func _exercise_marie_adult_reconquest() -> void:
	...
	_expect(bool(provider.confirm_scene_sequence().get("accepted", false)), "Marie scene completion preserves aftercare for J12")
	...
	_expect(provider.state.j11_pivot_outcome == "MARIE_ADULT_RECONQUEST", "Marie A5 records the exact semantic outcome")
	_expect(str(provider.state.obligations["aftercare_marie_j11"].get("status", "")) == "DUE", "Marie aftercare remains due through J11 closure")
	_confirm_transition(provider)
...
func _exercise_marie_non_adult_fallback() -> void:
	...
	_expect(provider.state.j11_pivot_outcome == "MARIE_NON_ADULT_RECONNECTION", "missing Marie gate records only the non-adult reconnection")
