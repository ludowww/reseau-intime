...
class_name R8CMediaProjectionPort
...
func supports_projection(projection_target: String) -> Dictionary:
	var supported := projection_target == SUPPORTED_TARGET
	return {
		"supported": supported,
...
func open(request: Dictionary) -> Dictionary:
	...
	if request["projection_target"] != SUPPORTED_TARGET:
		return _error_result(request["projection_target"], "UNSUPPORTED_PROJECTION")
	var beat := _beat_for_request(request)
	if beat.is_empty():
		return _error_result(SUPPORTED_TARGET, "AUTHORED_BEAT_MISMATCH")
	if beat["type"] != "MEDIA_REVEAL":
		return _error_result(SUPPORTED_TARGET, "UNSUPPORTED_BEAT_TYPE")
	...
	for existing in _open_requests:
		if ProjectionContracts.presentation_id_for(existing) != presentation_id:
			continue
		if existing != request:
			return _error_result(SUPPORTED_TARGET, "DIVERGENT_REQUEST")
		return _accepted_result(request, beat, true)
	if not _open_requests.is_empty():
		return _error_result(SUPPORTED_TARGET, "PROJECTION_ALREADY_OPEN")
...
func submit(command: Dictionary) -> Dictionary:
	...
	if command["kind"] != "CONTINUE":
		return _error_result(SUPPORTED_TARGET, "COMMAND_NOT_ALLOWED")
...
func acknowledge(receipt: Dictionary) -> Dictionary:
	...
	if not ProjectionContracts.validate_receipt_against_request(receipt, request)["valid"]:
		return _error_result(SUPPORTED_TARGET, "RECEIPT_IDENTITY_MISMATCH")
	var beat := _beat_for_request(request)
	var expected_kind := "VIEWED" if beat["content"]["requires_ack"] else "PRESENTED"
	if receipt["kind"] != expected_kind or receipt["subject_id"] != beat["content"]["media_id"]:
		return _error_result(SUPPORTED_TARGET, "UNEXPECTED_MEDIA_RECEIPT")
	for existing in _receipts:
		if existing == receipt:
			return _accepted_result(request, beat, true)
		if existing["presentation_id"] == receipt["presentation_id"]:
			return _error_result(SUPPORTED_TARGET, "DIVERGENT_RECEIPT")
...
func restore(snapshot_data: Dictionary) -> Dictionary:
	...
	for request in snapshot_data["open_requests"]:
		var beat := _beat_for_request(request)
		if request["projection_target"] != SUPPORTED_TARGET or beat.get("type") != "MEDIA_REVEAL":
			return {"accepted": false, "error_code": "INVALID_SNAPSHOT"}
	for receipt in snapshot_data["receipts"]:
		var request := _request_for_presentation_in(snapshot_data["open_requests"], receipt["presentation_id"])
		if request.is_empty():
			return {"accepted": false, "error_code": "INVALID_SNAPSHOT"}
		var beat := _beat_for_request(request)
		var expected_kind := "VIEWED" if beat["content"]["requires_ack"] else "PRESENTED"
...
func close(presentation_id: String) -> Dictionary:
	...
	return {
		"accepted": true,
		"idempotent": false,
		"projection_target": SUPPORTED_TARGET,
...
func _error_result(projection_target: String, error_code: String) -> Dictionary:
	return {
		"accepted": false,
		"idempotent": false,
		"projection_target": projection_target if projection_target == SUPPORTED_TARGET else "NONE",
...
func _beat_for_request(request: Dictionary) -> Dictionary:
	if (
		...
		or request.get("sequence_id") != _authored_sequence["sequence_id"]
		or request.get("authored_version") != _authored_sequence["authored_version"]
	):
		return {}
	for beat in _authored_sequence["beats"]:
		if beat["beat_id"] != request.get("beat_id"):
			continue
		if beat["type"] != request.get("beat_type") or beat["projection_target"] != request.get("projection_target"):
			return {}
		return beat
	return {}
...
func _request_for_presentation(presentation_id: String) -> Dictionary:
	return _request_for_presentation_in(_open_requests, presentation_id)


func _request_for_presentation_in(requests: Array, presentation_id: String) -> Dictionary:
