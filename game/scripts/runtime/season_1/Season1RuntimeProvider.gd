...
func on_thread_returned(thread_id: String) -> Dictionary: return active_provider.on_thread_returned(thread_id) if active_day in ["J02", "J03", "J04"] else {}
func presentation_count_by_id(id: String) -> int: return active_provider.presentation_count_by_id(id) if active_day in ["J02", "J03", "J04"] else _count_j01(id)
...
func content_end() -> Dictionary:
	if active_day == "J04" and j04_provider.phase == "complete": return j04_provider.runtime_map.get("day_end", {}).duplicate(true)
	return {}
...
func complete_pending_transition_flow(resume_action: String) -> Dictionary:
	...
	if resume_action == "start_day" and active_day in ["J02", "J03", "J04"] and active_provider.phase == "day_start_pending": return active_provider.start_day()
	return {"accepted": false}
...
func restore_snapshot(value: Dictionary) -> bool:
	...
	if version not in [2, SNAPSHOT_VERSION] or str(value.get("active_day", "")) not in ["J01", "J02", "J03", "J04"]: return false
	...
	if version == SNAPSHOT_VERSION and typeof(providers.get("J04")) != TYPE_DICTIONARY: return false
	...
	j04_snapshot = providers["J04"].duplicate(true); active_provider = j04_provider
	return true
