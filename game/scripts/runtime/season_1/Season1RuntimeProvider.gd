...
const J19_SCRIPT := preload("res://scripts/runtime/season_1/J19RuntimeProvider.gd")
const J20_SCRIPT := preload("res://scripts/runtime/season_1/J20RuntimeProvider.gd")
const J21_SCRIPT := preload("res://scripts/runtime/season_1/J21RuntimeProvider.gd")
...
func confirm_day_transition() -> Dictionary:
	...
	if active_day=="J18" and j18_provider.phase=="complete":
		if not _handoff_to_j19():return {"accepted":false}
		return {"accepted":true,"destination":"day_transition","presentation":j19_provider.day_start_presentation()}
	if active_day=="J19" and j19_provider.phase=="complete":
		if not _handoff_to_j20():return {"accepted":false}
		return {"accepted":true,"destination":"day_transition","presentation":j20_provider.day_start_presentation()}
	if active_day=="J20" and j20_provider.phase=="complete":
		if not _handoff_to_j21():return {"accepted":false}
		return {"accepted":true,"destination":"day_transition","presentation":j21_provider.day_start_presentation()}
...
func automatic_day_handoff() -> Dictionary:
	...
	if active_day=="J18" and j18_provider.phase=="complete":
		if not _handoff_to_j19():return {"accepted":false}
		var result:Dictionary=j19_provider.start_day();result.automatic_day_handoff=true;result.next_day_presentation=j19_provider.day_start_presentation();return result
	if active_day=="J19" and j19_provider.phase=="complete":
		if not _handoff_to_j20():return {"accepted":false}
		var result:Dictionary=j20_provider.start_day();result.automatic_day_handoff=true;result.next_day_presentation=j20_provider.day_start_presentation();return result
	if active_day=="J20" and j20_provider.phase=="complete":
		if not _handoff_to_j21():return {"accepted":false}
		var result:Dictionary=j21_provider.start_day();result.automatic_day_handoff=true;result.next_day_presentation=j21_provider.day_start_presentation();return result
...
func next_day_presentation() -> Dictionary:
	...
	if active_day=="J18":return DataLoader.load_json("res://data/runtime/season_1/j19_runtime_map.json").day_start.duplicate(true)
	if active_day=="J19":return DataLoader.load_json("res://data/runtime/season_1/j20_runtime_map.json").day_start.duplicate(true)
	if active_day=="J20":return DataLoader.load_json("res://data/runtime/season_1/j21_runtime_map.json").day_start.duplicate(true)
	return {}

func content_end() -> Dictionary:
	if active_day == "J21" and j21_provider.phase == "complete": return j21_provider.runtime_map.get("day_end", {}).duplicate(true)
	return {}
...
func pending_transition_flow() -> Dictionary:
	...
	if active_day=="J18":
		...
		if j18_provider.phase=="complete":return {"flow_phases":["NIGHT","NEW_DAY"],"next_day_presentation":next_day_presentation(),"resume_action":"automatic_day_handoff"}
	if active_day=="J19":
		if not j19_provider.pending_transition.is_empty():return j19_provider.pending_transition.duplicate(true)
		if j19_provider.phase=="day_start_pending":return {"flow_phases":["NEW_DAY"],"next_day_presentation":j19_provider.day_start_presentation(),"resume_action":"start_day"}
		if j19_provider.phase=="complete":return {"flow_phases":["NIGHT","NEW_DAY"],"next_day_presentation":next_day_presentation(),"resume_action":"automatic_day_handoff"}
	if active_day=="J20":
		if not j20_provider.pending_transition.is_empty():return j20_provider.pending_transition.duplicate(true)
		if j20_provider.phase=="day_start_pending":return {"flow_phases":["NEW_DAY"],"next_day_presentation":j20_provider.day_start_presentation(),"resume_action":"start_day"}
		if j20_provider.phase=="complete":return {"flow_phases":["NIGHT","NEW_DAY"],"next_day_presentation":next_day_presentation(),"resume_action":"automatic_day_handoff"}
	if active_day=="J21":
		if not j21_provider.pending_transition.is_empty():return j21_provider.pending_transition.duplicate(true)
		if j21_provider.phase=="day_start_pending":return {"flow_phases":["NEW_DAY"],"next_day_presentation":j21_provider.day_start_presentation(),"resume_action":"start_day"}
	return {}
...
func complete_pending_transition_flow(resume_action: String) -> Dictionary:
	...
	if resume_action == "start_day" and active_day in ["J02", "J03", "J04", "J05", "J06", "J07", "J08", "J09", "J10", "J11", "J12", "J13", "J14", "J15", "J16", "J17", "J18", "J19", "J20", "J21"] and active_provider.phase == "day_start_pending": return active_provider.start_day()
	return {"accepted": false}
...
func _handoff_to_j19()->bool:
	var candidate=J19_SCRIPT.new();if not candidate.initialize(state,j18_provider.transcripts_by_thread,j18_provider.produced_message_ids,j18_provider.unlocked_thread_ids,j18_provider.gallery_asset_ids):return false
	j19_provider=candidate;active_day="J19";active_provider=candidate;return true

func _handoff_to_j20()->bool:
	var candidate=J20_SCRIPT.new();if not candidate.initialize(state,j19_provider.transcripts_by_thread,j19_provider.produced_message_ids,j19_provider.unlocked_thread_ids,j19_provider.gallery_asset_ids):return false
	j20_provider=candidate;active_day="J20";active_provider=candidate;return true

func _handoff_to_j21()->bool:
	var candidate=J21_SCRIPT.new();if not candidate.initialize(state,j20_provider.transcripts_by_thread,j20_provider.produced_message_ids,j20_provider.unlocked_thread_ids,j20_provider.gallery_asset_ids):return false
	j21_provider=candidate;active_day="J21";active_provider=candidate;return true
...
func snapshot() -> Dictionary:
	return {"version": SNAPSHOT_VERSION, "active_day": active_day, "state": state.snapshot(), "provider_snapshots": {
		...
		"J19": j19_provider.snapshot() if j19_provider != null else {},
		"J20": j20_provider.snapshot() if j20_provider != null else {},
		"J21": j21_provider.snapshot() if j21_provider != null else {},
	}}
...
func restore_snapshot(value: Dictionary) -> bool:
	...
	active_provider=j18_provider
	if active_day=="J18":return true
	j19_provider=J19_SCRIPT.new();if not j19_provider.initialize(state,j18_provider.transcripts_by_thread,j18_provider.produced_message_ids,j18_provider.unlocked_thread_ids,j18_provider.gallery_asset_ids):return false
	if not j19_provider.restore_snapshot(providers.get("J19",{})):return false
	active_provider=j19_provider
	if active_day=="J19":return true
	j20_provider=J20_SCRIPT.new();if not j20_provider.initialize(state,j19_provider.transcripts_by_thread,j19_provider.produced_message_ids,j19_provider.unlocked_thread_ids,j19_provider.gallery_asset_ids):return false
	if not j20_provider.restore_snapshot(providers.get("J20",{})):return false
	active_provider=j20_provider
	if active_day=="J20":return true
	j21_provider=J21_SCRIPT.new();if not j21_provider.initialize(state,j20_provider.transcripts_by_thread,j20_provider.produced_message_ids,j20_provider.unlocked_thread_ids,j20_provider.gallery_asset_ids):return false
	if not j21_provider.restore_snapshot(providers.get("J21",{})):return false
	active_provider=j21_provider;return true
